...
Jira Cloudを、親子ツリー・設定可能な一覧表・時間記録で見やすくする軽量デスクトップアプリです。Rust + SlintでmacOS / Windowsを対象にします。

## 現在できること

- 親課題を選び、その配下だけに絞り込み
- キー、タイトル、本文、コメントの横断検索（Enterで実行）
- 表示列（担当者、期限、見積、消費時間、残余時間）の切り替え
- 表示列設定の端末内保存
- 課題のタイトル・本文・期限・初期見積もり更新
- 実カレンダーによる日付選択、開始日時と作業時間のworklog登録
- worklog登録時の残余時間自動減算、または登録後残余時間の手動指定
- 課題タイプによる一覧・ツリーの絞り込み
- 課題タイプ絞り込み中も一致する子孫を祖先付きで再帰表示
- 課題ツリーのお気に入り固定表示（サイト単位で端末内保存）
- `+` / `-`と「全部開く」「全部閉じる」による親子ツリーの展開・折りたたみ
- 一覧表のインデントと`+` / `-`による親子関係表示・展開・折りたたみ
- 課題タイプ・キー・タイトル・状態を独立列で左揃え表示
- ヘッダー境界のドラッグによる一覧列幅の変更
- 状態（完了以外・個別状態）による一覧の絞り込み
- 課題タイプと状態を色付きバッジで表示
- Jiraカスタムフィールドを表示列へ追加・削除（表示設定を端末内保存）
- boolean項目・Flagged・複数選択のJiraチェックボックス項目を一覧から一括編集
- 一覧の編集モード、明示的な一括保存、確認付きキャンセル
- メインウィンドウと左右ペインの自由なサイズ変更
- 左右ペインの個別開閉（一覧をウィンドウ全幅へ拡大可能）
- 右ペインの読み取り専用課題詳細と、ブラウザのJira課題ページを開くボタン
...
Secretはソースコード、Git、設定ファイルへ保存しないでください。短命のOAuthアクセストークンとサイト情報はメモリだけに保持し、ローテーション方式のリフレッシュトークンだけをmacOS KeychainまたはWindows Credential Managerへ保存します。同じ起動中の再同期ではメモリ上のセッションを再利用し、アクセストークンの期限切れ時だけ資格情報ストアを読み直します。Windows Credential Managerの1項目あたりのサイズ制限に対応するため、長いトークンは複数の資格情報へ分割します。
...
右ペインの課題内容は読み取り専用です。個別課題の編集は「Jiraで開く」からブラウザで行います。一覧の編集モードでは、タイトル、期限、初期見積もり、残余時間と、文字列・数値・日付・boolean型のJiraカスタムフィールドを一括編集できます。boolean型とFlaggedはチェックボックスで操作します。Jiraの複数選択チェックボックス項目は、セルの「選択...」から選択肢ごとにON/OFFできます。選択肢は課題の編集メタデータから取得するため、その課題で実際に利用可能な値だけをJiraへ送信します。フィールドのコンテキストや編集画面の対象外となる課題タイプでは、選択できない理由をダイアログに表示します。旧バージョンでキャッシュされたFlaggedと名前付きチェックボックスの型情報も起動時に自動移行します。ユーザーなどJira側の複雑な入力候補が必要な未対応フィールドは、誤った値を送らないため表示専用です。変更は「保存」を押すまでJiraへ送信されず、「キャンセル」では確認後に破棄されます。
...
3. ✅ 課題編集、日付更新、作業時間登録
4. ✅ ツリー・一覧の展開制御、状態フィルター、カスタム項目編集、残余時間管理
5. ⏸️ バーンダウンは実用性を再設計するまで保留
6. 🟡 macOS / Windows自動ビルド（完了）、認証ブローカー・署名・公証（配布資格情報の準備後）
