...
- boolean項目とJiraのFlagged項目をチェックボックスで一括編集
...
右ペインの課題内容は読み取り専用です。個別課題の編集は「Jiraで開く」からブラウザで行います。一覧の編集モードでは、タイトル、期限、初期見積もり、残余時間と、文字列・数値・日付・boolean型のJiraカスタムフィールドを一括編集できます。boolean型とFlaggedはチェックボックスで操作します。旧バージョンでキャッシュされたFlaggedの型情報も起動時に自動移行します。選択肢、ユーザー、配列などJira側の入力候補が必要な複雑なカスタムフィールドは、誤った値を送らないため表示専用です。変更は「保存」を押すまでJiraへ送信されず、「キャンセル」では確認後に破棄されます。
