...
右ペインの課題内容は読み取り専用です。個別課題の編集は「Jiraで開く」からブラウザで行います。一覧の編集モードでは、タイトル、期限、初期見積もり、残余時間と、文字列・数値・日付・boolean型のJiraカスタムフィールドを一括編集できます。boolean型とFlaggedはチェックボックスで操作します。Jiraの複数選択チェックボックス項目は、セルの「選択...」から選択肢ごとにON/OFFできます。選択肢は課題の編集メタデータから取得するため、その課題で実際に利用可能な値だけをJiraへ送信します。旧バージョンでキャッシュされたFlaggedと名前付きチェックボックスの型情報も起動時に自動移行します。ユーザーなどJira側の複雑な入力候補が必要な未対応フィールドは、誤った値を送らないため表示専用です。変更は「保存」を押すまでJiraへ送信されず、「キャンセル」では確認後に破棄されます。
